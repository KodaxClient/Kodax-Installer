...
public class InstallingSlide extends AbstractSlide {
    ...
    public void initialize(JFrame frame) {
        ...
        Multithreading.run(() -> {
            ...
            Manifest manifest = Manifest.fetch("https://raw.githubusercontent.com/KodaxClient/Kodax-Repo/master/installer.json");
            if (manifest == null) {
            ...
            File libraryDir = new File(mcDir, "libraries" + File.separator + "me" + File.separator + "kodingking" + File.separator + "Kodax" + File.separator + manifest.getLatestBuild());
            if (!libraryDir.exists())
                libraryDir.mkdirs();
            File libraryFile = new File(libraryDir, "Kodax-" + manifest.getLatestBuild() + ".jar");
            infoLabel.setText("Downloading Kodax...");
            progressBar.setValue(++progressValue);

            try {
                FileUtils.copyURLToFile(Settings.USE_BETA ? manifest.getLatestBetaDownload() : manifest.getBuildDownload(), libraryFile);
            } catch (IOException e) {
                e.printStackTrace();
            ...
            if (Settings.USE_OPTIFINE) {
                try {
                    ...
                    progressBar.setValue(++progressValue);
                    FileUtils.copyURLToFile(new URL("https://github.com/KodaxClient/Kodax-Repo/raw/master/resources/installer/OptiFine_1.8.9_HD_U_I7.jar"), optifineFile);
            ...
            File versionDir = new File(mcDir, "versions" + File.separator + "Kodax " + manifest.getLatestBuild());
