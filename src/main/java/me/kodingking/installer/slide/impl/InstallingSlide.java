...
public class InstallingSlide extends AbstractSlide {
    ...
    public void initialize(JFrame frame) {
        JProgressBar progressBar = new JProgressBar(1, 5 + (Settings.USE_OPTIFINE ? 2 : 0));
        ...
        Multithreading.run(() -> {
            ...
            Manifest manifest = Manifest.fetch("https://raw.githubusercontent.com/ProjectXENON/Xenon-Repo/master/installer.json");
            if (manifest == null) {
            ...
            File libraryDir = new File(mcDir, "libraries" + File.separator + "me" + File.separator + "kodingking" + File.separator + "ProjectXENON" + File.separator + manifest.getLatestBuild());
            if (!libraryDir.exists())
                libraryDir.mkdirs();
            File libraryFile = new File(libraryDir, "ProjectXENON-" + manifest.getLatestBuild() + ".jar");
            infoLabel.setText("Downloading Project XENON...");
            progressBar.setValue(++progressValue);

            try {
                FileUtils.copyURLToFile(manifest.getBuildDownload(), libraryFile);
            } catch (IOException e) {
                e.printStackTrace();
                infoLabel.setText("Error downloading Project XENON.");
                return;
            }

            File launchwrapperDir = new File(mcDir, "libraries" + File.separator + "net" + File.separator + "minecraft" + File.separator + "launchwrapper" + File.separator + "1.7");
            ...
            try {
                ...
                progressBar.setValue(++progressValue);
                FileUtils.copyURLToFile(new URL("https://libraries.minecraft.net/net/minecraft/launchwrapper/1.7/launchwrapper-1.7.jar"), launchwrapperFile);
            } catch (IOException e) {
                e.printStackTrace();
            ...
            if (Settings.USE_OPTIFINE) {
                try {
                    ...
                    progressBar.setValue(++progressValue);
                    FileUtils.copyURLToFile(new URL("https://github.com/ProjectXENON/Xenon-Repo/raw/master/resources/installer/OptiFine_1.8.9_HD_U_I7.jar"), optifineFile);

                    File optifineLibDir = new File(mcDir, "libraries" + File.separator + "optifine" + File.separator + "OptiFine" + File.separator + "1.8.9_HD_U_I7");
                    ...
                    progressBar.setValue(++progressValue);
                    URLClassLoader child = new URLClassLoader(new URL[]{optifineFile.toURL()}, this.getClass().getClassLoader());
                    Class classToLoad = Class.forName("optifine.Patcher", true, child);
                    Method method = classToLoad.getDeclaredMethod("main", String[].class);
                    method.invoke(null, new Object[]{new String[]{baseVersionJar.getAbsolutePath(), optifineFile.getAbsolutePath(), optifineLibJar.getAbsolutePath()}});
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            File versionDir = new File(mcDir, "versions" + File.separator + "ProjectXENON " + manifest.getLatestBuild());
            ...
            json = json
                    .replace("%FOLDER_NAME%", versionDir.getName())
                    .replace("%MAVEN_PATH%", "me.kodingking:ProjectXENON:" + manifest.getLatestBuild())
                    .replace("%TWEAK_CLASS%", "me.kodingking.xenon.injection.LaunchInjector")
                    .replace("%APPEND_HERE%", Settings.USE_OPTIFINE ? ",{\"name\": \"optifine:OptiFine:1.8.9_HD_U_I7\"}" : "");
            try {
                FileUtils.write(new File(versionDir, versionDir.getName() + ".json"), json);
            } catch (IOException e) {
                e.printStackTrace();
            ...
            infoLabel.setText("Installed! Create a profile with the created build.");
            progressBar.setValue(++progressValue);
        });
    }
    ...
    private File getMCDir() {
        if (OSValidator.isWindows())
            return new File(System.getProperty("user.home") + File.separator + "AppData" + File.separator + "Roaming" + File.separator + ".minecraft" + File.separator);
        else if (OSValidator.isMac())
            return new File(System.getProperty("user.home") + File.separator + "Library" + File.separator + "Application Support" + File.separator + "minecraft" + File.separator);
